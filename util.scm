...
(define (same-line? a b) (= (line-index a) (line-index b)))
(define (same-col? a b) (= (col-index a) (col-index b)))

(define (make-indexed-token tok beg-lci end-lci)
  (list tok beg-lci end-lci))
(define (token itok) (car itok))
(define (beg-lci itok) (cadr itok))
(define (end-lci itok) (caddr itok))
  ...
  (let ((buffer "")
    ...
    (define (read-line-if-needed)
	  ...
	  (let ((line (read-line port)))
	    (if (eof-object? line)
		line ; eof-object
		(begin (set! buffer line)
		       (set! line-num (1+ line-num))
		       (set! col-num 0)
		       (read-line-if-needed))))
	  #f))
    (define (read-char)
      (let ((res (read-line-if-needed)))
	(if (eof-object? res)
	    res
    ...
    (define (get-cur-pos)
      (make-line-col-index (if (= line-num -1) 0 line-num)
			   (if (= col-num -1) 0 col-num)))
    (define (dispatch n)
      (cond ((eq? n 'read-char) read-char)
	    ((eq? n 'peek-char) peek-char)
	    ((eq? n 'get-col-num) (lambda ()
				    (if (= col-num -1) 0 col-num)))
	    ((eq? n 'get-line-num) (lambda ()
				     (if (= line-num -1) 0 line-num)))
	    ((eq? n 'get-cur-pos) get-cur-pos)
...
(define (stream-get-cur-pos s) ((s 'get-cur-pos)))
...
(define (read-list stream)
  (define (read-list-without-paren)
    (let ((c (stream-peek-char stream)))
      (cond ((eof-object? c) (error "unbalanced parentheses"))
	    ((char=? #\) c)
	     (begin (stream-read-char stream)
		    '()))
	    ((char-set-member? char-set:whitespace c)
  ...
  (let ((c (stream-peek-char stream)))
    (if (char=? #\( c)
	(let* ((beg (stream-get-cur-pos stream))
	       (l (begin (stream-read-char stream)
			 (read-list-without-paren)))
	       (end (stream-get-cur-pos stream)))
...
(define (read-atom stream)
  (let ((beg (stream-get-cur-pos stream)))
    (define (read-atom-impl buffer end)
      (let* ((c (stream-peek-char stream))
	     (cur (stream-get-cur-pos stream)))
	(if (or (eof-object? c)
		(not (same-line? beg cur))
		(char-set-member? delimiters c))
	    (make-indexed-token
